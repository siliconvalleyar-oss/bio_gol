...
3. **Primero VERSION, después TAG** - El flujo CORRECTO es: (1) Actualizar archivo `VERSION` con la nueva versión sin la `v`, (2) Hacer commit de los cambios incluyendo el archivo VERSION, (3) Crear tag con la misma versión, (4) Push. Ejemplo: para versión 1.0.6 → PRIMERO cambiar VERSION a `1.0.6`, DESPUÉS commit + tag `v1.0.6` + push. Si no se actualiza VERSION primero, el tag no reflejará la versión real.
